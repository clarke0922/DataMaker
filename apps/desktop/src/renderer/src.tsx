...
import { DatabaseOutlined, DashboardOutlined, SafetyCertificateOutlined, SearchOutlined, SettingOutlined } from '@ant-design/icons';
import type { MetadataStatsDto, SearchHitDto, SystemInfoDto } from '@datamaker/contracts';
  ...
  return <Layout className="shell">
    <Layout.Sider width={224} theme="light" className="sider">
      ...
      <Menu mode="inline" defaultSelectedKeys={['dashboard']} items={[
        ...
        { key: 'metadata', icon: <DatabaseOutlined />, label: '元数据' },
        { key: 'quality', icon: <SafetyCertificateOutlined />, label: '质量中心' },
        { key: 'settings', icon: <SettingOutlined />, label: '系统管理' }
      ]} />
    </Layout.Sider>
    <Layout>
      <Layout.Header className="header"><Typography.Title level={4}>元数据工作台</Typography.Title><Tag color="blue">本地模式</Tag></Layout.Header>
      <Layout.Content className="content">
        {error && <Alert closable onClose={() => setError('')} type="error" message={error} />}
        {!info ? <Spin /> : !info.initialized && <Alert type="warning" showIcon message="系统尚未初始化管理员账户" description="首个开发迭代将从此状态进入管理员初始化向导。" />}
